...
function ScholarStats() {
    ...
    useEffect(() => {
        fetch('/scholar-stats.json?t=' + Date.now())
            ...
            .then(data => {
                setStats({
                    citations: data.citations ? parseInt(data.citations).toLocaleString() : '--',
    ...
    return (
        <div className="flex flex-wrap justify-center gap-2 mb-6">
            <div className="inline-flex h-5 rounded-[3px] overflow-hidden shadow-sm">
                <span className="bg-neutral-500 text-white px-2 text-[11px] leading-5 font-medium">
                    Citations
                </span>
                <span className="bg-[#4c8bf5] text-white px-2 text-[11px] leading-5 font-semibold">
                    {stats.citations}
                </span>
            </div>
...
export default function Profile({ author, social, features, researchInterests }: ProfileProps) {
    ...
    useEffect(() => {
        ...
        const userHasLiked = localStorage.getItem('your-website-user-liked');
    ...
    const handleLike = () => {
        ...
        if (newLikedState) {
            localStorage.setItem('your-website-user-liked', 'true');
            setShowThanks(true);
            setTimeout(() => setShowThanks(false), 2000);
        } else {
            localStorage.removeItem('your-website-user-liked');
            setShowThanks(false);
        }
    };
    ...
    return (
        ...
        >
            ...
            <div className="relative w-64 h-64 mx-auto mb-6">
                <div className="w-64 h-64 rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-105">
                    <Image
                        src={author.avatar}
                        alt={author.name}
                        width={256}
                ...
                {/* My Kitty Arrow */}
                <div className="absolute -left-24 top-1/2 transform -translate-y-1/2 flex items-center gap-0.5">
                    <span className="text-sm font-medium text-accent whitespace-nowrap">My Kitty</span>
                    <svg className="w-19 h-6 text-accent" viewBox="0 0 64 24" fill="none" xmlns="http://www.w3.org/2000/svg">
